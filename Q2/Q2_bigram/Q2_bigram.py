...
# using probabilities for bigrams instead of just one word
...
def get_tf_icf_matrix(X_t, Y_c):
    ...
    for i in range(len(X_t)):
        ...
        for j in range(1, len(X_t[i].split())-1):
            # bigram
            # print(len(X_t[i].split())-1, len(X_t))
            word = X_t[i][j-1] + " " + X_t[i][j]
            words.add(word)
            if word not in tf:
                tf[word] = {}
...
if __name__ == "__main__":
    ...
    json.dump(tf, open("D:\\IR\\Q2_bigram\\tf.json", "w"))
    json.dump(icf, open("D:\\IR\\Q2_bigram\\icf.json", "w"))
    ...
    f = open("D:\\IR\\Q2_bigram\\final.txt", 'a')
    f.write(outputstr)
    f.write("\n")
    for t in output:
        f.write(t)
