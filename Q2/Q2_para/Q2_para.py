...
# scores for each category while testing is multiplied instead of adding, without taking logarithm
# if testing word is not present in training vocabulary, we add 0.5 to category score instead of 0
# adding category probabilty to score
...
def test(X_train, y_train, X_test, y_test, tf, icf, category_wordcount):
    ...
    category_predict = []
    for doc in X_test:
        json.dump(doc.split(), open("D:\\IR\\Q2_para\\testdata.json", "a"))
            ...
            for word in doc.split():
                if word in tf:
                    if category in tf[word]:
                        ...
                        icf_word = icf[word]
                        # print(math.log10(tf_word*icf_word))
                        scores_for_category[category] *= tf_word*icf_word
                                     
                    else:
                        tf_word = 0
                        icf_word = 0
                        scores_for_category[category] += 0.5
                else:
                    continue
...
if __name__ == "__main__":
    ...
    json.dump(tf, open("D:\\IR\\Q2_para\\tf.json", "w"))
    json.dump(icf, open("D:\\IR\\Q2_para\\icf.json", "w"))
    ...
    f = open("D:\\IR\\Q2_para\\final.txt", 'a')
    f.write(outputstr)
    f.write("\n")
    for t in output:
        f.write(t)
